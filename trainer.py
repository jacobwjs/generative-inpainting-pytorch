...
class Trainer_DDP(nn.Module):
    def __init__(self,
                 config,
                 use_cuda=True,
                 local_rank=0):
        
        super(Trainer_DDP, self).__init__()
        self.config = config
        ...
        # Generator
        #
        self.netG = Generator(config['netG'], use_cuda=True, device_id=local_rank)
...
#         )
        ...
        # Discriminator
        #
        self.localD = LocalDis(self.config['netD'], use_cuda=True, device_id=local_rank)
        self.globalD = GlobalDis(self.config['netD'], use_cuda=True, device_id=local_rank)
...
#         self.train()
        l1_loss = nn.L1Loss() #.cuda(self.local_rank)
        losses = {}
...
#         )
        ...
        batch_output = self.localD(batch_data)
...
#         )
        ...
        batch_output = self.globalD(batch_data)
...
#                 torch.mean(global_fake_pred) * self.config['global_wgan_loss_alpha']
    ...
    ### TODO:
    ### - expose this for datadistributedparallel
    ###
